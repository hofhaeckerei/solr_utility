<?php
namespace H4ck3r31\SolrUtility\IndexQueue;

/*
 * This file is part of the TYPO3 CMS project.
 *
 * It is free software; you can redistribute it and/or modify it under
 * the terms of the GNU General Public License, either version 2
 * of the License, or any later version.
 *
 * For the full copyright and license information, please read the
 * LICENSE.txt file that was distributed with this source code.
 *
 * The TYPO3 project - inspiring people to share!
 */

use ApacheSolrForTypo3\Solr\IndexQueue\SerializedValueDetector;
use H4ck3r31\SolrUtility\ContentObject\BaseCategoryContentObject;
use TYPO3\CMS\Core\SingletonInterface;

class SerializedValueHook implements SerializedValueDetector, SingletonInterface
{
    /**
     * @param array $indexingConfiguration
     * @param string $solrFieldName
     * @return bool
     */
    public function isSerializedValue(array $indexingConfiguration, $solrFieldName)
    {
        return (
            $indexingConfiguration[$solrFieldName]
            === BaseCategoryContentObject::CONTENT_OBJECT_NAME
            || !empty($indexingConfiguration[$solrFieldName . '.']['_isSerialized'])
        );
    }
}
